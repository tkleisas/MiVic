...
What it does, in order: the body is a **proxy** fitted to a targeted basemesh, and the
face is pushed towards the reference by nine weighted detail targets; the skin is
`old_caucasian_male`, scaled by 0.78 because the pack paints for MakeHuman's own
lighting and the face otherwise arrives clipped; the pack's clothes are repainted into
one grey — a closed-front field jacket that a placket, a line of gold buttons and a pair
of red collar tabs, all built as geometry by `tools/blender/makehuman_uniform.py`, turn
into the tunic of the reference portrait; the hair is repainted a dark grey and its
hairline given the reference's temple peaks; and the moustache is a **mesh** from the
community bodyparts06 pack, widened and drooped, over paint worked into the skin map —
the paint is measured off the mesh rather than a UV layout, because the head is found
from the head bone and in the rest pose the fingers reach further forward than the nose
does. The pipe is authored, not found: `tools/blender/makehuman_pipe.py` sweeps a shank
and lathes a bowl and weights every vertex to the head bone, so it is smoked, not held.
`base.obj` is a stand-in that is never meant to be seen and is deleted before export.
...
this repository, so nothing has to be downloaded to build or run it. One part of
one model is the exception: the briefing figure's moustache is
`grinsegold_moustache` by **grinsegold**, from the MakeHuman community bodyparts06
asset pack, shared under **CC-BY** — the credit is here, and the mesh is edited
(widened, drooped and repainted) as the licence allows. The MakeHuman system
assets the figure is otherwise built from are CC0. The
