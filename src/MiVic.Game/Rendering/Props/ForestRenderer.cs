...
public sealed class ForestRenderer : IDisposable
{
    ...
    private void Load(string baseDirectory)
    {
        ...
        foreach (string file in Species)
        {
            ...
            try
            {
                ...
                MeshData baked = Bake(model, out float height);
    ...
    /// <summary>
    /// Flattens a model's node hierarchy into one mesh in the model's own space, and
    /// reports how tall the result is.
    /// </summary>
    private static MeshData Bake(ModelData model, out float height)
    {
        List<VertexPositionNormal> vertices = [];
        List<ushort> indices = [];

        // Parts arrive parents-first, so a parent's transform is always known by
        // the time its children are walked.
