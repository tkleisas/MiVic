"""What the imported MakeHuman head is given that the asset pack does not supply.

The pack ships clothes, hair, eyes, eyebrows, teeth and skins, and no facial hair at
all — ten hair assets and not one moustache. The figure this briefing is built around is
recognised by its moustache, so it has to be made rather than found.

It is painted into the skin map rather than modelled as geometry, and the reason is that
a painted moustache cannot come loose. Geometry would need a position on the face, a
weight to the head bone, and a material of its own; paint needs a UV and nothing else,
and it deforms with the skin for free because it *is* the skin. Every pixel is placed by
measuring the mesh in front of it, which matters because the mapping from the face to
the texture is an asset pack's business and not something to assume:
