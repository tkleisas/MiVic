...
Models are **low-poly, stylised, and faction-tinted**: a per-vertex material shade
is multiplied by a per-instance faction colour, so one red tint still shows dark
tracks, light panels and gun metal. All current assets come from Quaternius (see
`src/MiVic.Game/Content/Models/README.md`); the loader is a plain glTF 2.0 reader,
so any CC0/QAL-compatible source can be dropped in.

Known limitation: models render in their **bind pose**. Skeletal animation is
future work.
